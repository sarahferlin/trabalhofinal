...
O monitoramento dos municípios que compõem a área de estudo, resultou, no período analisado, no registro global de `r nviag_lc` viagens de pesca. Os aparelhos de pesca mais representativos em termos de número de viagens no período foram `r ord_3_app_lc` (Figura @fig-aparelhoslc).
...
O total capturado no período foi de `r capt_total_lc` t. Os volumes de captura por categoria de pescado encontram-se na Tabela @fig-capttotallc. As principais categorias de pescado do período foram `r ord_3_spp_lc`.
...
O monitoramento dos municípios que compõem a área de estudo resultou, no período analisado, no registro global de `r nviag_ls$n_descargas` viagens de pesca. Os aparelhos de pesca mais representativos em termos de número de viagens no período foram `r top_3_aparelhos_ls` (Figura @fig-aparelhoslitoralsul).
...
O total capturado no período foi de `r capturatotal_ls` t. Os volumes de captura por categoria de pescado encontram-se na Tabela @fig-capttotallitoralsul. As principais categorias de pescado do período foram `r top_3_pescados_ls`.
