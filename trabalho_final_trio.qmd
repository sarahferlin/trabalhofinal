...
valortotal_ln <- round(sum(dados_ln$valor / 1000000 , 1))
...
O monitoramento dos municípios que compõem a área de estudo, resultou, no período analisado, no registro global de `r nviag_ln` viagens de pesca. Os aparelhos de pesca mais representativos em termos kg descarregado no período foram `r nome_aparelhos` (Figura @fig-aparelhosln).

```{r fig-aparelhosln}
#| fig-cap: "Tabela com todos os aparelhos de pesca descarregados no Litoral Norte de Julho a Dezembro de 2022"
...
O total capturado no período foi de `r capturatotal_ln` t. Os volumes de captura por categoria de pescado encontram-se na Tabela @fig-capttotalln. As principais categorias de pescado do período foram `r nome_especies`.

```{r fig-capttotalln}
#| fig-cap: "Tabela com todas as catergorias de pescado descarregadas no Litoral Norte de Julho a Dezembro de 2022"
...
Considerando-se o preço de primeira comercialização, estimou-se que toda a captura do período gerou uma receita de aproximadamente R\$ `r valortotal_ln` milhões.
...
capturatotal_ls <- sum(dados_ls$kg_periodo,na.rm=T)/1000
...
O total capturado no período foi de `r capturatotal_ls` t. Os volumes de captura por categoria de pescado encontram-se na Tabela @capttotallitoralsul. As principais categorias de pescado do período foram `r nome_3_principais_capturas_litoralsul`.
...
Considerando-se
o preço de primeira comercialização, estimou-se que toda a captura do período gerou uma receita de aproximadamente R$ `r valortotal_litoralsul`.
